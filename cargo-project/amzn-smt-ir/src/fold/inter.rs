...
/// An `InterLogicFolder<T>` is a type that can be used to transform SMT terms from logic `T` to
/// logic `Self::U`. This a more specialized version of [`Folder`] that can provide more default
/// method implementations.
pub trait InterLogicFolder<T: Logic>: Sized {
    /// Logic that terms will be transformed into.
    type U: Logic;
    ...
    /// Transforms a term into logic `U`, recursively transforming its contents.
    /// By default, calls [`SuperFold::super_fold_with`].
    fn fold_term(&mut self, term: Term<T>) -> Result<Term<Self::U>, Self::Error> {
        term.super_fold_with(self)
    }

    /// Transforms a constant into logic `U`.
    fn fold_const(&mut self, constant: IConst) -> Result<Term<Self::U>, Self::Error> {
        Ok(constant.into())
    }

    /// Transforms a variable into logic `U`.
    fn fold_var(&mut self, var: IVar<T::Var>) -> Result<Term<Self::U>, Self::Error>;

    /// Transforms a Core operation into logic `U`, recursively transforming its arguments.
    /// By default, calls [`SuperFold::super_fold_with`].
    fn fold_core_op(&mut self, op: ICoreOp<T>) -> Result<Term<Self::U>, Self::Error> {
        op.super_fold_with(self).map(Into::into)
    }

    /// Transforms a non-Core operation into logic `U`, recursively transforming its arguments.
    fn fold_theory_op(&mut self, op: IOp<T>) -> Result<Term<Self::U>, Self::Error>;

    /// Transforms an uninterpreted function into logic `U`, recursively transforming its arguments.
    fn fold_uninterpreted_func(&mut self, func: IUF<T>) -> Result<Term<Self::U>, Self::Error>;

    /// Transforms a let binder into logic `U`, recursively folding its arguments.
    /// By default, calls [`SuperFold::super_fold_with`].
    fn fold_let(&mut self, l: ILet<T>) -> Result<Term<Self::U>, Self::Error> {
        l.super_fold_with(self).map(Into::into)
    }

    /// Transforms a match binder into logic `U`, recursively folding its arguments.
    /// By default, calls [`SuperFold::super_fold_with`].
    fn fold_match(&mut self, m: IMatch<T>) -> Result<Term<Self::U>, Self::Error> {
        m.super_fold_with(self).map(Into::into)
    }

    /// Transforms a quantifier into logic `U`, recursively folding its arguments.
    fn fold_quantifier(&mut self, quantifier: IQuantifier<T>)
        -> Result<Term<Self::U>, Self::Error>;

    /// Transforms a set-logic command.
    /// By default, returns as-is.
    fn fold_set_logic(&mut self, logic: ISymbol) -> Result<Command<Term<Self::U>>, Self::Error> {
        Ok(Command::SetLogic { symbol: logic })
    }

    /// Transforms an asserted term.
    /// By default, calls [`Self::fold_term`].
    fn fold_assert(&mut self, asserted: Term<T>) -> Result<Command<Term<Self::U>>, Self::Error> {
    ...
    fn fold_declare_const(
        ...
        symbol: ISymbol,
        sort: ISort,
    ) -> Result<Command<Term<Self::U>>, Self::Error> {
...
impl<T: Logic, F: InterLogicFolder<T>> Folder<T, Marker> for F {
    type Output = Term<F::U>;
    type Error = F::Error;
